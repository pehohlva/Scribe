...
QMap<QString, QPen> FoRegion::penstyle() {
  ...
  plist.insert(QString("0.5mm solid"),
               QPen(Qt::black, MM_TO_POINT(0.5), Qt::SolidLine));
  plist.insert(QString("0.5mm dotline"),
               QPen(Qt::black, MM_TO_POINT(0.5), Qt::DotLine));
  for (int i = 1; i < 9; ++i) {
    plist.insert(QString("%1mm solid").arg(i),
                 QPen(Qt::black, MM_TO_POINT(i), Qt::SolidLine));
    plist.insert(QString("%1mm dotline").arg(i),
                 QPen(Qt::black, MM_TO_POINT(i), Qt::DotLine));
    plist.insert(QString("%1mm dashline").arg(i),
                 QPen(Qt::black, MM_TO_POINT(i), Qt::DashLine));
  }
  return plist;
}
...
void FoRegion::styleContainer(QDomElement e, const qreal height) {
  ...
  if (height != 0) {
    e.setAttribute(
        "height", metrics(qAbs(height - margin_top - padding - margin_bottom)));
  }
}
...
QString FoRegion::hash() const {
  ...
  const QString header = QString("%1|%2|%3|%4|")
                             .arg(margin_top)
                             .arg(margin_bottom)
                             .arg(margin_right)
                             .arg(margin_left);
  const QString margin =
      QString("%1|%2|%3").arg(bg.name()).arg(bog.name()).arg(border);
...
QByteArray M_PageSize::hashmodel() const {
  ...
  const QString header = QString("%1|%2|%3|%4|")
                             ...
                             .arg(G_regt.height())
                             .arg(modus);
  const QString margin = QString("%1|%2|%3|%4")
                             .arg(body.margin_top)
                             .arg(body.margin_bottom)
                             .arg(body.margin_right)
                             .arg(body.margin_left);
...
M_PageSize::operator QVariant() const { return QVariant::fromValue(*this); }
...
QPointF M_PageSize::FooterInitPoints(const int index) {
  ...
  const qreal spacepage = index * InterSpace;
  return QPointF(body.margin_left,
                 fromTopY + spacepage + Ytop + FooterHeaderPadding);
}
...
qreal M_PageSize::faktor() {
  return qMax(G_regt.width(), G_regt.height()) /
         qMin(G_regt.width(), G_regt.height());
}
...
qreal M_PageSize::height() const {
  return pageBoundingRect().height() - body.margin_bottom - body.margin_top +
         0.5; ////  -bottom - top
}

qreal M_PageSize::width() const {
  return pageBoundingRect().width() - body.margin_left - body.margin_right +
         0.5; /////////  -lef , - right
}

QSizeF M_PageSize::internal_size() { return QSizeF(width(), height()); }

void M_PageSize::SetMargin(FoRegion e) { body = e; }
...
QRectF M_PageSize::pageBoundingRect() const {
  if (landscape) {
    return QRectF(0, 0, qMax(G_regt.height(), G_regt.width()),
                  qMin(G_regt.height(), G_regt.width()));
  } else {
    return QRectF(0, 0, qMin(G_regt.height(), G_regt.width()),
                  qMax(G_regt.height(), G_regt.width()));
  }
}
...
QRectF M_PageSize::PageInternal(const int index) {
  ...
  const qreal spacepage = index * InterSpace;
  return QRectF(
      QPointF(body.margin_left, fromTopY + spacepage + body.margin_top),
      internal_size());
}
...
QRectF M_PageSize::PrintArea() {
  return QRectF(QPointF(body.margin_left, body.margin_top),
                QSizeF(width(), G_regt.height() * MaximumPages));
}
...
void M_PageSize::ReportPage(QDomElement e) {
  ...
  e.setAttribute("page-width",
                 QString("%1mm").arg(Pointo(G_regt.width(), "mm")));
  e.setAttribute("page-height",
                 QString("%1mm").arg(Pointo(G_regt.height(), "mm")));
...
QRectF M_PagesizeMake(QPrinter::PageSize psize, bool landscape) {
  ...
  const qreal faktor =
      qMax(pa.width(), pa.height()) / qMin(pa.width(), pa.height());
...
QDomDocument M_PageSize::fopMeta() {
  ...
  QDomProcessingInstruction header = dom.createProcessingInstruction(
  ...
  basexslforoot.setAttribute("xmlns:cms",
                             "http://www.pulitzer.ch/2007/CMSFormat");
  basexslforoot.setAttribute("xmlns:fox",
  ...
  if (area[0].enable) {
    ...
    b_txt_1.appendChild(
        dom.createComment(QString("Header region / xsl-region-before ")));
  ...
  if (area[1].enable) {
    ...
    b_txt_2.appendChild(
        dom.createComment(QString("Footer region / xsl-region-after ")));
  ...
  if (area[2].enable) {
    ...
    b_txt_3.appendChild(
        dom.createComment(QString("Left  region / xsl-region-start ")));
  ...
  if (area[3].enable) {
    ...
    b_txt_4.appendChild(
        dom.createComment(QString("Right  region / xsl-region-end ")));
