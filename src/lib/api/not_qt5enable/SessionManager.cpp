...
ApiSession::ApiSession() : pdev(new QPrinter(QPrinter::ScreenResolution)) {
  ...
  FormatRegister(QT_TR_NOOP("B5 (176 x 250 mm, 6.93 x 9.84 inches)"),
                 QPrinter::B5);
  ...
  /////////////qDebug() << "### current format faktor name " <<
  /// current_Page_Format.faktor() << "-" << current_Page_Format.HName();
  //////////////qDebug() << "### current format margin hash" <<
  /// current_Page_Format.body << "-" << current_Page_Format.hashmodel();
}

void ApiSession::clearKey() { uniqueid.clear(); }
