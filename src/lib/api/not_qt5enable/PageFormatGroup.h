...
Qt::SolidLine, Qt::PenCapStyle cap = Qt::SquareCap, Qt::PenJoinStyle join =
Qt::BevelJoin )
...
inline QDebug operator<<(QDebug debug, FoRegion &udoc) {
  debug.nospace() << "FoRegion(Enable." << udoc.enable << ",top(),"
                  << udoc.margin_top << ",bottom()," << udoc.margin_bottom
                  << ",right()," << udoc.margin_right << ",left(),"
                  << udoc.margin_left << ",PenWi()," << udoc.rpen.widthF()
                  << ",PenStyle()," << udoc.rpen.style() << ")";
  return debug.space();
}
...
public:
  ...
  {
    const qreal LargeDoc =
        G_regt.width() - body.margin_right - body.margin_left;
    const qreal InnHightDoc =
        G_regt.height() - body.margin_top - body.margin_bottom;
    return QRectF(body.margin_left, body.margin_top, LargeDoc, InnHightDoc);
  }

  QRectF headerRect() /* page 0 move to topLeft()  point */
  {
    return QRectF(0, 0, width() + body.margin_right + body.margin_left,
                  body.margin_top);
  }
  ...
  {
    return QRectF(0, 0, body.margin_left,
                  height() + body.margin_top + body.margin_bottom);
  }

  QRectF endRect() /* page 0 move to rightLeft()  point */
  {
    return QRectF(0, 0, body.margin_right,
                  height() + body.margin_top + body.margin_bottom);
  }
  ...
  /*  | region-before = header 0 |  region-after = footer 1 | region-start =
   * left  2 |> region-end = right  3 >*/
