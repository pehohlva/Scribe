...
public:
  ...
  ///// OOReader::otextedit
  OOReader(const QString file = QString(), WIDGEDEST e = otextbrowser,
           QObject *parent = 0);
...
private:
  ...
  bool iterateElements(const QDomElement e, QTextCursor &cur,
                       const int processing);
  bool convertTable(QTextCursor &cur, const QDomElement e, const int processing,
                    bool roottable = false);
  bool convertCellTable(const QDomElement e, QTextCursor &cur,
                        const int processing);
  bool convertBlock(QTextCursor &cur, QDomElement e, const int processing);
  bool convertList(QTextCursor &cur, QDomElement e, const int processing,
                   int level = 1, const QString classname = QString("deafult"));
  bool convertFragment(QTextCursor &cur, const QDomElement e,
                       QTextCharFormat parent = QTextCharFormat(),
                       bool HandleSpace = false);
  bool convertSpaceTag(QTextCursor &cur, const QDomElement e,
                       QTextCharFormat parent, bool HandleSpace = false);
  void insertTextLine(QTextCursor &cur, QStringList line,
                      QTextCharFormat parent = QTextCharFormat(),
                      bool HandleSpace = false);
  bool convertFrame(QTextCursor &cur, const QDomElement e,
                    QTextCharFormat parent, bool HandleSpace);
  bool convertImage(QTextCursor &cur, const QDomElement e,
                    QTextCharFormat parent, bool HandleSpace = false);
  QTextFrameFormat
  FrameFormat(const QString name); /* format from css name class */
  /* read css2 format */
  QTextCharFormat charFormatCss2(const QString name,
                                 QTextCharFormat parent = QTextCharFormat(),
                                 bool HandleSpace = false);
  QPair<QTextBlockFormat, QTextCharFormat>
  paraFormatCss2(const QString name,
  ...
  void FrameImageFormatPaint(const QString name,
                             const QDomElement e); /* image / frame */
  /* table column cell row format container + unknow */
  void UnknowFormatPaint(const QString name, const QString style_name,
                         const QDomElement e);

  QTextBlockFormat
  TextBlockFormFromDom(const QDomElement e,
                       QTextBlockFormat pf = QTextBlockFormat());
  QTextCharFormat TextCharFormFromDom(const QDomElement e,
                                      QTextCharFormat pf = QTextCharFormat());
