...
public:
  ...
  inline int picskilo() { return data.size(); }
  QByteArray streams() { return qUncompress(data); }
...
inline QDebug operator<<(QDebug debug, SPics &udoc) {
  debug.nospace() << "SPics(name." << udoc.name << "," << udoc.info << ",size()"
                  << udoc.data.size() << ",type()" << udoc.extension << ")";
  return debug.space();
}
...
public:
  ...
  RichDoc() {
    html = QByteArray("<p style=\" margin-top:0px; margin-bottom:0px; "
                      "margin-left:0px; margin-right:0px; -qt-block-indent:0; "
                      "text-indent:0px;\">Your text</p>");
    style = QString(
        "min-height:300px;padding:0px 0px 0px 0px;background-color:#ffffff;");
    nullmargin = false;
  }
  void margin(bool e) { nullmargin = e; }
  ...
  QString Hxtml() { return QString(html.constData()); }

  bool isAbsolute() { return style.contains("position:absolute;"); }
  QTextDocument *todoc();
  bool nullmargin;
  QString style;
...
public:
  ...
  operator QVariant() const { return QVariant::fromValue(*this); }
  int size() { return Listening.size(); }
