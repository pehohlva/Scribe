...
//
#include "../xslt_convert.h" //// zip open
#include "../scribe_config.h"
#include "../basic_config.h" //// cm mm x to point px
#include <QDomDocument>

OOReader::OOReader(const QString file, OOReader::WIDGEDEST e, QObject *parent)
    : OOFormat(), maxStyleCount(0), styleCurrentCount(0), charsRead(),
      QTWRITTELN(false), construct_modus(e), sumOfBlocks(0), fileHash("250186"),
      fontTotal(0) {

   SCDEBUG() << "start OOReader here... file " << file;
  if (qApp->thread() != QThread::currentThread()) {
    SCDEBUG() << "### Heya speed other  QThread as Apps....";
  }
  odtzipfile = file;
  reset();
}
...
void OOReader::read() {
  ...
  SCDEBUG() << "### OOReader:" << __FUNCTION__ << ":" << __LINE__;
...
void OOReader::openStreams(QMap<QString, QByteArray> list) {
  ...
  SCDEBUG() << "### style read from id " << fileHash;
  SCDEBUG() << "### css2.size() -> " << maxStyleCount
           << "--------------------------------------------------------<";
  SCDEBUG() << "### total block count to progress -----> " << sumOfBlocks;
...
bool OOReader::convertBody(const QDomElement &element) {
  ...
  while (!child.isNull()) {
    ...
    /////////convertTable( QTextCursor &cur , QDomElement e  , const int
    ///processing )
  }
  return true;
  ///// cursor.movePosition(QTextCursor::End);
...
bool OOReader::iterateElements(const QDomElement e, QTextCursor &cursor,
...
bool OOReader::convertList(QTextCursor &cur, QDomElement e,
                           const int processing, int level,
                           const QString classname) {
  SCDEBUG() << "### convertList begin " << processing << " name " << e.tagName()
           << " l." << level;
  ...
  QDomElement ul_blck =
      e.firstChildElement("text:list-item"); /* grab next from this */
...
OOReader::paraFormatCss2(const QString name, QTextBlockFormat parent,
                         bool HandleSpace) {
  QTextBlockFormat paraFormat = OODefaultMargin();
...
QTextCharFormat OOReader::charFormatCss2(const QString name,
...
bool OOReader::convertBlock(QTextCursor &cur, QDomElement e,
                            const int processing) {
...
bool OOReader::convertFrame(QTextCursor &cur, const QDomElement e,
                            QTextCharFormat parent, bool HandleSpace) {
  if (e.isNull()) {
    return false;
  }
...
bool OOReader::convertImage(QTextCursor &cur, const QDomElement e,
                            QTextCharFormat parent, bool HandleSpace) {
  if (e.isNull()) {
    return false;
  }
  ...
  const QString TimestampsMs = QString("%1-%2%3-graphic-frame")
                                   .arg(timer1.toTime_t())
                                   .arg(timer1.toString("zzz"))
                                   .arg(imageCurrentCount);

  //////////////SCDEBUG() << "### Init Frame image ID  " << TimestampsMs;
  ...
  if (domdd.size() > 0 || QTWRITTELN) {
    /* load dom to rotate image mirror or saved on style load  qt not save image
    on style not need .... */
    /* default image */
    ...
    if (!drawimage.isNull()) {
      Qdoc->addResource(QTextDocument::ImageResource, QUrl(TimestampsMs),
                        drawimage);
    }
...
bool OOReader::convertSpaceTag(QTextCursor &cur, const QDomElement e,
                               QTextCharFormat parent, bool HandleSpace) {
  if (e.isNull()) {
    return false;
  }
...
bool OOReader::convertFragment(QTextCursor &cur, const QDomElement e,
                               QTextCharFormat parent, bool HandleSpace) {
  if (e.isNull()) {
    return false;
  }
  ...
  if (e.tagName() == QLatin1String("text:span")) {
    styleroot.merge(charFormatCss2(e.attribute("text:style-name", "default"),
                                   parent, HandleSpace));
    cur.setCharFormat(styleroot);
    if (e.text().isEmpty()) {
      cur.insertText(QString(QChar::Nbsp), parent);
  ...
  //////////////SCDEBUG() << "### convertFragment  fragment tag in  ->" <<
  ///e.tagName();
  ...
  while (!child.isNull()) {
    ...
    if (child.isElement()) {
      const QDomElement es = child.toElement();
      ////////SCDEBUG() << "### loop fragment  tag ->" << es.tagName();
...
void OOReader::insertTextLine(QTextCursor &cur, QStringList line,
                              QTextCharFormat parent, bool HandleSpace) {
...
void OOReader::styleNameSetup(const QByteArray chunk, const QString label) {
  ...
  while ((iPosition = expression.indexIn(QString(chunk), iPosition)) != -1) {
    ...
    valueName = valueName.mid(searchPara.size() + 1,
                              valueName.size() - searchPara.size() - 2);
...
Qt::Alignment OOReader::TagAlignElement(const QDomElement e) {
  ...
  if (e.attribute("fo:vertical-align") == "middle" ||
      e.attribute("fo:display-align") == "center") {
...
OOReader::StyleInfo::StyleInfo(const StyleInfo &d) { operator=(d); }
...
OOReader::StyleInfo::StyleInfo()
    : name("erno"), position(0), type(obodypage), valid(false), filled(false),
      ofchar(OODefaultCharFormats(true)) {
  QTextFormat page;
...
void ChildImport::copyDeep(QIODevice *device, QXmlStreamWriter &out) {
  ...
  while (!atEnd()) {
    ...
    if (isStartElement()) {
      ...
      /////SCDEBUG() << "### copydeep element tagname   ---------- " <<
      ///name().toString();
      /////////SCDEBUG() << "### copydeep element tagname   ---------- " <<
      ///prefix().toString();
  ...
  if (erno != 0) {
    //////qWarning() << "### ChildImport::copyDeep error    ---------- " <<
    ///error();
  } else {
    out.writeEndElement();
  }
...
QTextBlockFormat OOReader::TextBlockFormFromDom(const QDomElement e,
                                                QTextBlockFormat pf) {
  ...
  for (int i = 0; i < attlist.count(); i++) {
    ...
    if (nod.nodeName().toLower() == "fo:background-color") {
      pf.setBackground(
          QBrush(FoCol->foColor(nod.nodeValue(), FopColor::Transparent)));
    }

    if (nod.nodeName().toLower() == "fo:space-after" ||
        nod.nodeName().toLower() == "fo:margin-bottom") { /* bottom */
      pf.setBottomMargin(Unit(nod.nodeValue()));
    }

    if (nod.nodeName().toLower() == "fo:space-start" ||
        nod.nodeName().toLower() == "fo:margin-right") { /* right */
      pf.setRightMargin(Unit(nod.nodeValue()));
    }

    if (nod.nodeName().toLower() == "fo:space-end" ||
        nod.nodeName().toLower() == "fo:margin-left") { /* left */
...
QTextCharFormat OOReader::TextCharFormFromDom(const QDomElement e,
                                              QTextCharFormat pf) {
  charsRead++;
  bool boldText = false;
  ...
  if (!nameFont.isEmpty()) {
    ...
    SCDEBUG() << "### userfont.exactMatch()  " << userfont.exactMatch()
             << nameFont2;
    pf.setFont(userfont);
  } else {
  ...
  for (int i = 0; i < attlist.count(); i++) {
    ...
    if (nod.nodeName().toLower() == "fo:color") {
      pf.setForeground(
          QBrush(FoCol->foColor(nod.nodeValue(), FopColor::DarkColor)));
    }

    ////if (nod.nodeName().toLower() == "fo:font-family" ||
    ///nod.nodeName().toLower() == "style:font-name") {
  ...
  const qreal pointfont = qMax(Unit(e.attribute("fo:font-size")),
                               Unit(e.attribute("font-size-complex")));
  if (pointfont > 2) {
  ...
  if (!e.attribute("style:text-scale").isEmpty() ||
      !e.attribute("fo:letter-spacing").isEmpty()) {
    QString value =
        qMax(e.attribute("style:text-scale"), e.attribute("fo:letter-spacing"));
  ...
  //////////// style:text-underline-width="auto" fo:font-size="11pt"
...
void OOReader::TextBlockFormatPaint(const QString name, const QDomElement e) {
  ...
  if (css2[name].valid) {
    ...
    if (!es.isNull()) {
      css2[name].ofchar =
          TextCharFormFromDom(es, OODefaultCharFormats(QTWRITTELN));
    }
  ...
  ///////////SCDEBUG() << "### TextBlockFormatPaint  " << e.tagName() << name;
}
...
void OOReader::TextCharFormatPaint(const QString name, const QDomElement e) {
  ...
  QTextCharFormat Charformat =
      TextCharFormFromDom(e, OODefaultCharFormats(QTWRITTELN));
  if (css2[name].valid) {
    css2[name].filled = true;
#ifdef _OOREADRELEASE_
  ...
  SCDEBUG() << "### TextCharFormatPaint  " << e.tagName() << name;
}
...
void OOReader::FrameImageFormatPaint(const QString name, const QDomElement e) {
  ...
  if (!im.isNull()) {
    QColor bg = FoCol->foColor(im.attribute("fo:background-color"),
                               FopColor::Transparent);
    int goalpha =
        OoColorAlpha(QString(im.attribute("style:background-transparency"))
                         .replace("%", "")
                         .toInt());
    bg.setAlpha(goalpha);
    QTextFrameFormat fox;
...
void OOReader::TextListFormatPaint(const QString name, const QDomElement e) {
  ...
  if (nx.attribute("style:num-format") == QString::fromLatin1("1") &&
      nd.isNull()) {
    style = QTextListFormat::ListDecimal;
  } else if (nx.attribute("style:num-format") == QString::fromLatin1("a") &&
             nd.isNull()) {
  ...
  ///////////SCDEBUG() << "### CharBull set   " << CharBull  << " - " <<
  ///nd.attribute("text:bullet-char","z");
}

void OOReader::UnknowFormatPaint(const QString name, const QString style_name,
                                 const QDomElement e) {
#ifdef _OOREADRELEASE_
  if (css2[name].valid) {
    css2[name].css = cssGroup(e);
  ...
  if (style_name == QLatin1String("table-column")) {
    /* save column */
    const qreal width =
        Unit(e.firstChildElement("style:table-column-properties")
                 .attribute("style:column-width"));
    SCDEBUG()
        << "### style:table-column-properties  widthwidthwidthwidthwidthwidth  "
        << width;
    ...
    if (css2[name].valid) {
      ...
      ///////////qWarning() << "Not register style name ->" << style_name <<
      ///name;
    }
    ...
    ///////////////SCDEBUG() << "### table width  ->" << width  <<  " real" <<
    ///tablecss.attribute("style:width");
    ...
    if (css2[name].valid) {
      ...
      table.setBorder(0.2);
      table.setBackground(QBrush(FoCol->foColor(
          tablecss.attribute("fo:background-color"), FopColor::Transparent)));
      css2[name].of = table;
#ifdef _OOREADRELEASE_
      css2[name].chunk = CatDomElement(e);
      css2[name].css = cssGroup(e);
    ...
    cell.setBottomPadding(qMax(Unit(celcss.attribute("fo:padding")),
                               Unit(celcss.attribute("fo:padding-bottom"))));
    cell.setRightPadding(qMax(Unit(celcss.attribute("fo:padding")),
                              Unit(celcss.attribute("fo:padding-right"))));
    cell.setTopPadding(qMax(Unit(celcss.attribute("fo:padding")),
                            Unit(celcss.attribute("fo:padding-top"))));
    cell.setLeftPadding(qMax(Unit(celcss.attribute("fo:padding")),
                             Unit(celcss.attribute("fo:padding-left"))));

    cell.setBackground(QBrush(FoCol->foColor(
        celcss.attribute("fo:background-color"), FopColor::Transparent)));
    cell.setProperty(CellBorderStyleID, celcss.attribute("fo:border"));

    //////SCDEBUG() << "### table-cell color st size ->" <<
    ///celcss.attribute("fo:background-color").size();
...
void OOReader::registerFontDoc(const QDomElement &element) {
  ...
  SCDEBUG() << "### registerFontDoc  standardFont.name  -> "
           << standardFont.defaultFamily();
  SCDEBUG() << "### registerFontDoc  standardFont.name  -> "
           << standardFont.family();
  SCDEBUG() << "### registerFontDoc  standardFont.name  -> "
    ...
    if (child.isElement()) {
      ...
      if (el.tagName() == QLatin1String("style:font-face")) {
        ...
        if (!fonts.isEmpty()) {
          fontTotal++;
          if (fontTotal == 1) {
            standardFont = QFont(el.attribute("svg:font-family"), 15);
          }
  ...
  while (!child.isNull()) {
    if (child.isElement()) {
      ...
      if (el.tagName() == QLatin1String("style:style")) {
        ...
        SCDEBUG() << "### forType ......" << forType;

        if (forType == QLatin1String("text")) {
          TextCharFormatPaint(sname,
                              el.firstChildElement("style:text-properties"));
        } else if (forType == QLatin1String("paragraph")) {
          TextBlockFormatPaint(
              sname, el.firstChildElement("style:paragraph-properties"));
...
bool OOReader::convertCellTable(const QDomElement e, QTextCursor &cur,
                                const int processing) {
...
bool OOReader::convertTable(QTextCursor &cur, const QDomElement e,
                            const int processing, bool roottable) {
  /* 1441 from fop convert */
  ///////SCDEBUG() << "### convertTable.---" << styleCurrentCount << sumOfBlocks
  ///<< " tag " << e.tagName();
  //////return true;
  ...
  while (!column.isNull()) {
    QTextLength cool_wi =
        QTextLength(QTextLength::PercentageLength, defaultPercents);
    bool appendC = false;
    const QString sname = column.attribute("table:style-name");
    const int makecolls =
        qMax(column.attribute("table:number-columns-repeated").toInt(), 1);
    if (!sname.isEmpty()) {
      /* QTextFrameFormat */
  ...
  for (int i = 0; i < constraints.size(); ++i) {
    ...
    if (modcool.rawValue() == defaultPercents) {
      constraints.replace(
          i, QTextLength(QTextLength::PercentageLength, coolsPercents));
    }
  }
    ...
    if (line.isElement()) {
      ...
      if (oorow.tagName() == "table:table-row") {
        ...
        while (!oocell.isNull()) {
           ...
           */
          if (oocell.tagName() == QLatin1String("table:covered-table-cell") ||
              increment) {
            gocool++;
            ...
            /* normal td cell */
            const int cr = qMax(
                oocell.attribute("table:number-rows-spanned").trimmed().toInt(),
                0);
            const int cs = qMax(oocell.attribute("table:number-columns-spanned")
                                    .trimmed()
                                    .toInt(),
                                0);
            if (cs > 0) {
            ...
            if (cell.isValid()) {
              ...
              const QString name =
                  oocell.attribute("table:style-name", "default");
              if (css2[name].valid) {
              ...
              qWarning() << "!!!! not valid cell at->  row:" << gorow
                         << " column:" << gocool;
            }
          }
