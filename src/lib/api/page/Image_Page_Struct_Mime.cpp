...
QTextDocument *RichDoc::todoc() {
  ...
  while (i.hasNext()) {
    ...
    d->addResource(QTextDocument::ImageResource, QUrl(record.name),
                   record.pix());
  }
      ...
      for (de = para.begin(); !(de.atEnd()); ++de) {
        ...
        if (fr.isValid()) {
          ...
          if (base.isAnchor()) {
            ...
            helper.setPosition(fr.position() + fr.length(),
          ...
          if (pico.isValid()) {
            ...
            ////////////qDebug() << "### from RichDoc add resource " <<
            /// hrefadress;
...
QUrl SPics::indoc() {
  ...
  return QUrl(
      QString("./%1.%2").arg(name).arg(QString(extension.data()).toLower()));
}
...
QList<SPics> OpenImageGroup(const QString datastream_base64) {
  ...
  QByteArray bytes(
      QByteArray::fromBase64(xcode)); /* decoded base64 string to QByteArray */
  QBuffer buffer(&bytes);
