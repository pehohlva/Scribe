...
QString cssGroup(const QDomElement e) {
   SCDEBUG() << "### OOFormat:" << __FUNCTION__ << ":" << __LINE__;
  QStringList cssitem;
  QDomNamedNodeMap alist = e.attributes();
  for (int i = 0; i < alist.count(); i++) {
    QDomNode nod = alist.item(i);
    cssitem.append(
        QString("%1:%2").arg(nod.nodeName().toLower()).arg(nod.nodeValue()));
  }
  QDomNode child = e.firstChild();
  while (!child.isNull()) {
    if (child.isElement()) {
      ...
      for (int x = 0; x < list.count(); x++) {
        ...
        cssitem.append(QString("%1:%2")
                           .arg(nod.nodeName().toLower())
                           .arg(nod.nodeValue()));
      }
    }
    child = child.nextSibling();
...
QString ootrimmed(QString txt, const QString txttransform) {
  ...
  if (txt.size() >= 1) {
    if (txt.at(0) == QChar::Null ||
        txt.at(0) == QChar(' ') | txt.at(0) == QChar::Nbsp) {
      firstspace = true;
    }
    if (txt.at(cosize) == QChar::Null ||
        txt.at(cosize) == QChar(' ') | txt.at(cosize) == QChar::Nbsp) {
      lastspace = true;
    }
