...
qreal FopInt(const QString datain) {
  ...
  //////////qDebug() << "### request unit data->" << datain << " size->" <<
  ///datain.size();
  qreal points = -1;
...
qreal Unit(qreal unit, const QString unita) { return Pointo(unit, unita); }
qreal ToUnit(qreal unit, const QString unita) { return Pointo(unit, unita); }
...
/* os path from sistem */
QString PathConvert(QString path) { return QDir::toNativeSeparators(path); }
...
QString getGSDefaultExeName() {
  ...
  for (int i = 0; i < gsonly.size(); ++i) {
    ...
    if (RealName.contains("Ghostscript")) {
      ...
      for (int e = 1; e < 99; e++) {
        ...
        if (softs.value(RealName + "/" + gVersion + "/GS_DLL")
                .toString()
                .size() > 6) {
          Pinfo.setFile(
              softs.value(RealName + "/" + gVersion + "/GS_DLL").toString());
  ...
  QString pathfinder = getGSLinuxPath(
      QString("gs")); /* which  simple append systems path mac + linux */
  if (pathfinder.size() > 2 && pathfinder.endsWith("gs", Qt::CaseInsensitive)) {
    return pathfinder;
  }

  QStringList paths;
  paths << "/usr/local/bin/gs"
        << "/usr/bin/gs"
        << "/bin/gs"
        << "/sw/bin/gs"
        << "/opt/bin/gs";
...
#endif
  /* forum
   * http://www.qtcentre.org/forum/f-qt-programming-2/t-qsettings-read-only-avaiable-10254.html
   */
}
...
double getGSVersion() {
  ...
  if (!process.waitForFinished()) {
    ...
    if (pos1 > 0) {
      Stversion = QString("%1.%2")
                      .arg(regExp1.cap(1).toInt())
                      .arg(regExp1.cap(2).toInt());
...
QPixmap LoadPS(QString fn, const QString arguments_append) {
  ...
  const QString qttmpFile =
      _GSCACHE_ + QString("%2_%1.png")
                      .arg(timer1.toString("yyyy-MM-dd-HH-mm-ss-zzz"))
                      .arg(letter);
